...
use app\cms\model\Plot as PlotModel;

class Plot extends Admin
{
    public function index()
    {
        ...
        // 数据列表
        $data_list = PlotModel::where($map)->order($order)->paginate();
    ...
    public function add()
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($inform = PlotModel::create($data)) {
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->isPost()) {
            // 表单数据
            $data = $this->request->post();
            if (PlotModel::update($data)) {
                $this->success('编辑通知成功', 'index');
            } else {
                $this->error('编辑通知失败');
            }
        }
        $info = PlotModel::get($id);
        // 显示添加页面
        // 显示添加页面
        return ZBuilder::make('form')
            ->addFormItems([
